...
Function Confirm-BestPracticesWithPPU { 
		...
		Process { 
            Try {
                ...
                $BPAArg = "TabularEditor.exe """"Provider=MSOLAP;Data Source=$($PBIEndpoint)/$($WorkspaceName);Connect Timeout=120;User ID=$($Username);Password=$($Password)"""" """"$($DatasetName)"""" -A """"$($BPAUrl)"""" -E -W -V -T """"$($OutputFile)""""" 
                Write-Host $BPAArg
