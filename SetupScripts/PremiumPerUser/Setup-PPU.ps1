...
$SampleModelURL = "https://github.com/kerski/pbi-dataops-template/blob/part10/Pbi/SampleModel/SampleModel.pbix?raw=true"
...
$ProjectResult = az devops project create `
                --name $ProjectName `
                --description "Part 10 example of Bringing DataOps to Power BI" `
...
$PipelineResult = az pipelines create --name $PipelineName --repository-type "tfsgit" `
                --description "Part 10 example pipeline of Bringing DataOps to Power BI" `
                --org "$($AzDOHostURL)$($LogInfo.name)" `
                --project $ProjectName `
